...
2. **Euclidean Distance Transform (EDT)**: Compute the shortest distance from each non-zero pixel to the background.
3. **Seed Placement**: In *coarse* mode (`separation=False`), one seed per connected component at its distance-transform maximum. In *fine* mode (`separation=True`), seeds are spaced adaptively based on the component's own EDT, allowing touching features to be split automatically.
4. **Watershed Segmentation**: Use seeds as markers and segment the image based on the EDT gradient field.
...
- **Velocity**: Derived from central finite differences of barycenter positions (edge-clamped).
- and many other
...
l_thr =  # Intensity threshold[Gauss] (float) (used for basin contours)
...
st.track_all(datapath, cores, l_thr, m_size, dx, dt, sign, separation, verbose, doppler)
```
